...
function addCadreTVAs($letras)
{
    ...
    $this->SetXY( $r1+2, $y2 - 8 );
    $this->Cell(2,1, $letras);
}
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $impuesto, $total_venta,$subtotal, $descuento, $simbolo )
{
    $this->SetFont('Arial','',8);
    ...
    $length = $this->GetStringWidth( round($subtotal,2) );
    $this->Cell( $length, 2, number_format($subtotal,2));

    // calculando el iva y mostrando
    $this->SetXY( $rei, $y1+10 );
    $length = $this->GetStringWidth( $simbolo );
    $this->Cell( $length, 2, $simbolo);

    $length = $this->GetStringWidth(round($impuesto,2) );
    $this->Cell( $length, 2, number_format($impuesto,2));

    // calculando el descuento y mostrando
    $this->SetXY( $rei, $y1+15 );
    $length = $this->GetStringWidth( $simbolo );
    $this->Cell( $length, 2, $simbolo);

    $length = $this->GetStringWidth(round($descuento,2) );
    $this->Cell( $length, 2, number_format($descuento,2));
    ...
    $this->Cell( $length, 2, number_format($total_venta,2));
