...
public function insertar($idproveedor,$idusuario,$idempresa,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_compra,$idarticulo,$cantidad,$precio_compra,$precio_venta){
	$sql="INSERT INTO ingreso (idproveedor,idusuario,idempresa,tipo_comprobante,serie_comprobante,num_comprobante,fecha_hora,impuesto,total_compra,estado) VALUES ('$idproveedor','$idusuario','$idempresa','$tipo_comprobante','$serie_comprobante','$num_comprobante','$fecha_hora','$impuesto','$total_compra','Aceptado')";
...
function getConsecutivoIngreso($tipo,$idempresa) {
	$sql= "CALL SP_CONSECUTIVO_INGRESO('$tipo','$idempresa')";
	return ejecutarConsulta($sql);
