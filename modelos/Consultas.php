...
public function ventasfechacliente($fecha_inicio,$fecha_fin,$idcliente,$idempresa){
	$sql="SELECT DATE(v.fecha_hora) as fecha, u.nombre as usuario, p.nombre as cliente,v.serie_comprobante, v.num_comprobante , v.total_venta, v.impuesto, v.estado FROM venta v INNER JOIN persona p ON v.idcliente=p.idpersona INNER JOIN usuario u ON v.idusuario=u.idusuario WHERE DATE(v.fecha_hora)>='$fecha_inicio' AND DATE(v.fecha_hora)<='$fecha_fin' AND v.idcliente='$idcliente' AND v.idempresa = '$idempresa'";
	return ejecutarConsulta($sql);
}
...
public function infoempresa($idempresa){
	$sql="SELECT idempresa,nombre,tipo_documento,num_documento,direccion,telefono,email,imagen,condicion FROM empresa WHERE idempresa='$idempresa'";
	return ejecutarConsulta($sql);
}

public function getempresa($nombre){
	$sql="SELECT idempresa,nombre,tipo_documento,num_documento,direccion,telefono,email,imagen,condicion FROM empresa WHERE nombre='$nombre'";
	return ejecutarConsulta($sql);
}
